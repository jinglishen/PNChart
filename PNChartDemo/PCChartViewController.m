...
//        [self.barChart setStrokeColors:@[PNGreen,PNGreen,PNRed,PNGreen,PNGreen,PNGreen,PNRed,PNGreen]];
        [self.barChart setStrokeColor:PNBlue];
...
- (void)userClickedOnBarAtIndex:(NSInteger)barIndex
{
    ...
    PNBar *selectdBar = [self.barChart.bars objectAtIndex:barIndex];
    for (PNBar *bar in self.barChart.bars) {
        if (selectdBar == bar) {
            bar.barColor = PNWhite;
        } else {
            bar.barColor = PNBlue;
        }
    }
...
//    
//    [selectdBar.layer addAnimation:animation forKey:@"Float"];
}
