...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        CGRect barViewFrame = frame;
        barViewFrame.origin = CGPointMake(5, 15);
        barViewFrame.size = CGSizeMake(CGRectGetWidth(frame) - 10, CGRectGetHeight(frame) - 15);
        _barView = [[UIView alloc] initWithFrame:barViewFrame];
...
{
    _copyGrade = grade;
    CGFloat startPosY = (1 - grade) * _barView.frame.size.height;
    
    UIBezierPath *progressline = [UIBezierPath bezierPath];
    
    [progressline moveToPoint:CGPointMake(_barView.frame.size.width / 2.0, _barView.frame.size.height + 15)];
    [progressline addLineToPoint:CGPointMake(_barView.frame.size.width / 2.0, startPosY)];
    ...
    if (_grade) {
        ...
        if (_barColorGradientStart) {
            ...
            self.gradientMask.lineWidth    = _barView.frame.size.width;
            self.gradientMask.frame = CGRectMake(0, 0, _barView.frame.size.width, _barView.frame.size.height);
            self.gradientMask.path = progressline.CGPath;
            
            CAGradientLayer *gradientLayer = [CAGradientLayer layer];
            gradientLayer.startPoint = CGPointMake(0.0,0.0);
            gradientLayer.endPoint = CGPointMake(1.0 ,0.0);
            gradientLayer.frame = CGRectMake(0, 0, _barView.frame.size.width, _barView.frame.size.height);
...
{
    CGFloat topSpace = _barView.frame.size.height * (1-self.grade);
    CGFloat textWidth = self.frame.size.width;
    
    [self.layer addSublayer:self.textLayer];
    [self.textLayer setFontSize:10];
    
    [self.textLayer setString:[[NSString alloc]initWithFormat:@"￥%0.2f",grade*self.maxDivisor]];
    ...
    float verticalY = topSpace - size.height + 12;
...
- (void)setIsNegative:(BOOL)isNegative{
    if (isNegative) {
        ...
        frame.origin.x = (self.frame.size.width - size.width)/2.0;
        frame.size = size;
